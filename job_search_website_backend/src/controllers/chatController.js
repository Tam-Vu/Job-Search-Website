...
class ChatController {
  ...
  async createConversation(req, res) {
    try {
      ...
      console.log('userId:', userId, receiverId, isGroup);
  ...
  async addMembersToConversation(req, res) {
    try {
      ...
      if (response.EC === 0) {
        ...
        const conversationMembers = await chatService.getConversationMembers(
          conversationId
        );
  ...
  async markAsSeen(req, res) {
    try {
      ...
      const response = await chatService.markConversationAsSeen(
        conversationId,
        userId
      );
