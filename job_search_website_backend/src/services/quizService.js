...
class QuizService {
    ...
    try {
      ...
      await db.sequelize.transaction(async (t) => {
        for (const questionData of questions) {
          ...
          if (
            questionData.questionType === 'SelectField' &&
            questionData.choices &&
            questionData.choices.length > 0
          ) {
            const choicesWithQuestionId = questionData.choices.map(
              (choice) => ({
  ...
  getQuizAssignment = async (assignmentId, employeeId) => {
    try {
      ...
      if (
        ...
        assignment.correctAnswers !== null &&
        assignment.totalQuestions !== null
      ) {
        assignment.dataValues.scoreDisplay = `${assignment.correctAnswers}/${assignment.totalQuestions}`;
        assignment.dataValues.percentageScore =
    ...
    try {
      ...
      await db.sequelize.transaction(async (t) => {
        for (const answer of answers) {
          const question = assignment.quiz.questions.find(
  ...
  getQuizResults = async (quizId, employerId) => {
    try {
      ...
      const results = assignments.map((assignment) => {
        ...
        if (
          plainAssignment.quizAnswers &&
          plainAssignment.quizAnswers.length > 0
        ) {
          const correctAnswers = plainAssignment.quizAnswers.filter(
  ...
  getQuizResultDetails = async (assignmentId, userRole, userId) => {
    try {
          ...
          include: [
            {
              ...
              include: [{ model: db.employers, attributes: ['companyName'] }],
              raw: false,
              nest: true,
            },
            {
      ...
      if (
        ...
        plainAssignment.quizAnswers = plainAssignment.quizAnswers.map(
          (answer) => {
  ...
  getEmployeesForQuizAssignment = async (
    quizId,
    employerId,
    searchQuery = ''
  ) => {
    try {
      ...
      const formattedEmployees = employees.map((employee) => {
        ...
        const hasAssignment =
          plainEmployee.quizAssignments &&
          plainEmployee.quizAssignments.length > 0;
        ...
        return {
          ...
          assignmentStatus: hasAssignment
            ? plainEmployee.quizAssignments[0].status
            : null,
          assignmentId: hasAssignment
            ? plainEmployee.quizAssignments[0].id
            : null,
          completedAt: hasAssignment
            ? plainEmployee.quizAssignments[0].completedAt
            : null,
          score:
            hasAssignment &&
            plainEmployee.quizAssignments[0].correctAnswers !== null
