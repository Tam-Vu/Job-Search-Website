    ...
    try {
      ...
      await db.sequelize.transaction(async (t) => {
        for (const questionData of questions) {
          ...
          if (questionData.questionType === 'SelectField' && 
  ...
  getQuizAssignmentsByQuizId = async (quizId, employerId) => {
    try {
      ...
      const formattedAssignments = assignments.map(assignment => {
        ...
        if (plainAssignment.correctAnswers !== null && plainAssignment.totalQuestions !== null) {
          plainAssignment.scoreDisplay = `${plainAssignment.correctAnswers}/${plainAssignment.totalQuestions}`;
          plainAssignment.percentageScore = (plainAssignment.correctAnswers / plainAssignment.totalQuestions) * 100;
        } else {
...
const totalQuestionsCount = assignment.quiz.questions ? assignment.quiz.questions.length : 0;
...
if (assignment.quiz && assignment.quiz.questions) {
  assignment.quiz.questions.forEach(question => {
    const userAnswer = assignment.quizAnswers.find(answer => answer.questionId === question.id);
    ...
    if (userAnswer) {
      ...
      if (question.questionType === 'SelectField') {
        questionResult.Answer = {
          ...
          choiceText: userAnswer.choice ? userAnswer.choice.choiceText : null,
        };
      } else if (question.questionType === 'TextField') {
        questionResult.Answer = {
...
const percentageScore = totalQuestionsCount > 0 ? (correctAnswersCount / totalQuestionsCount) * 100 : 0;
  ...
  getDetailedQuizAnswers = async (assignmentId) => {
    ...
    return {
      ...
      DT: {
        correctAnswers: correctAnswersCount,
        totalQuestions: totalQuestionsCount,
        percentageScore: totalQuestionsCount > 0 ? (correctAnswersCount / totalQuestionsCount) * 100 : 0,
